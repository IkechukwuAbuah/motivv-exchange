...
describe('Exchange', () => {
    ...
    describe('Withdrawing Tokens', () =>{
        ...
        describe ('Success',() => {
            beforeEach(async () => {
                ...
                //Now withdraw Tokens
                transaction = await exchange.connect(user1).withdrawToken(token1.address, amount) // connects user1's wallet to deposit token funciton 
                result = await transaction.wait() 
            })
            ...
            describe ('Failure', () => {
                //failure code goes here
                    it('- fails for insufficent funds', async()=>{
                        // Attempt to withdaw tokens withut depositing
    ...
    describe('Checking Balances', () =>{
        let transaction, result
        let amount = tokens(1) //We deposit 10 tokens in this test
    ...
    describe ('Order actions', async() => {
        ...
        beforeEach(async() =>{
            ...
            //Give user2 tokens to fill orders with 
        ...
        describe ('Cancelling orders', async ()=> {
            describe ('Success', async() => {
                beforeEach(async() =>{
                    //user1 deposits tokens
                    transaction = await exchange.connect(user1).cancelOrder(1) //cancelled order(1) here is the order ID
                    result = await transaction.wait()
                })

                it('- updates canceled orders', async () =>{
                    expect(await exchange.orderCancelled(1)).to.equal(true) // orderCancelled(1) order ID
                })
        ...
        describe ('Filling orders', async ()=> {
            ...
            describe('Success',() => {
                ...
                it('- executes the trade and chages fees', async () => {
                    //Check balances for tokengive
                ...
                it('- emits a Trade event', async () => {
                    ...
                    const args = event.args 
                    expect(args.id).to.equal(1)
                    expect(args.user).to.equal(user2.address) //filling the trade
                    expect(args.tokenGet).to.equal(token2.address)
                    expect(args.amountGet).to.equal(tokens(1))
                    expect(args.tokenGive).to.equal(token1.address)
                    expect(args.amountGive).to.equal(tokens(1))
            ...
            describe('Failure',() => {
                ...
                it('- rejects already filled order', async () => {
                    transaction = await exchange.connect(user2).fillOrder(1) //order 1 is filled
                    await transaction.wait()

                    await expect(exchange.connect(user2).fillOrder(1)).to.be.reverted //we try to fill the order again
                })

                it('- rejects cancelled orders', async () => {
                    transaction = await exchange.connect(user1).cancelOrder(1) //order 1 is cancelled
                    await transaction.wait()

                    await expect(exchange.connect(user2).fillOrder(1)).to.be.reverted //try to fill the cancelled order
                })
            })
        })   
    })
